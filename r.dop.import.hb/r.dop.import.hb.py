...
############################################################################
#
# MODULE:      r.dop.import.hb
# AUTHOR(S):   Johannes Halbauer, Anika Weinmann, Leon Louwarts
# PURPOSE:     Downloads DOPs for Bremen, Bremerhaven and AOI
...
# %module
# % description: Downloads DOPs for Bremen, Bremerhaven and AOI.
# % keyword: raster
# % keyword: import
# % keyword: DOP
...
def main():
    """Main function of r.dop.import.hb"""
    global rm_vectors
    aoi = options["aoi"]
    ...
    if options["memory"]:
        grass.warning(
            _(
                ...
                "module for HB do not accept a <memory> parameter.",
            ),
        )
    ...
    if flags["k"]:
        grass.warning(
            _(
                "-k flag will be ignored, beacuse HB DOPs will be imported "
    ...
    grass.message(_("Creating DOP tiles for HB..."))
    ...
    try:
        ...
        for tile in tiles_list:
            ...
            r_dop_import_worker_hb = Module(
                "r.dop.import.worker.hb",
                **param,
                run_=False,
            )
            # catch all GRASS output to stdout and stderr
            r_dop_import_worker_hb.stdout_ = grass.PIPE
            r_dop_import_worker_hb.stderr_ = grass.PIPE
            queue.put(r_dop_import_worker_hb)
        queue.wait()
    except Exception:
