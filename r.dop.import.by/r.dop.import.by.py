...
############################################################################
#
# MODULE:      r.dop.import.by
# AUTHOR(S):   Johannes Halbauer, Anika Weinmann, Leon Louwarts
# PURPOSE:     Downloads DOPs for Bayern and AOI
...
# %module
# % description: Downloads DOPs for Bayern and AOI.
# % keyword: raster
# % keyword: import
# % keyword: DOP
...
def main():
    """Main function of r.dop.import.by"""
    global rm_vectors
    aoi = options["aoi"]
    ...
    if options["memory"]:
        grass.warning(
            _(
                ...
                "module for BY do not accept a <memory> parameter.",
            ),
        )
    ...
    if flags["k"]:
        grass.warning(
            _(
                "-k flag will be ignored, because BY DOPs will be imported "
    ...
    grass.message(_("Creating DOP tiles for BY..."))

    # set tile size in map units (meter)
    tile_size = 1000

    # set grid name
    grid = f"tmp_grid_BY_{ID}"
    ...
    try:
        grass.message(
            _(f"Importing {number_tiles} DOPs for BY in parallel..."),
        )
        for tile in tiles_list:
            ...
            r_dop_import_worker_by = Module(
                "r.dop.import.worker.by",
                **param,
                run_=False,
            )
